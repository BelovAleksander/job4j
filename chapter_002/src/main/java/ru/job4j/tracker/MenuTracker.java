...
public class MenuTracker {
    ...
    public final void fillActions(final StartUI ui) {
        this.actions[position++] = this.new AddItem("Add new item.", 0);
        this.actions[position++] = new MenuTracker.ShowItems("Show all items.", 1);
        this.actions[position++] = this.new EditItem("Edit item.", 2);
...
class ExitProgram extends BaseAction {
     ...
     */
    public ExitProgram(final String name, final int key, final StartUI ui) {
        super(name, key);
        this.ui = ui;
    }
    ExitProgram(final String name, final int key) {
        super(name, key);
    }
    /**
     * Метод содержит действия, требуемые для обработки
